...
public class BatchTests : TestBase
{
    ...
    {
        CacheGetBatchResponse response = await client.GetBatchAsync(client, null!, new List<byte[]>());
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheGetBatchResponse.Error)response).ErrorCode);
        response = await client.GetBatchAsync(client, "cache", (List<byte[]>)null!);
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheGetBatchResponse.Error)response).ErrorCode);
        var badList = new List<byte[]>(new byte[][] { Utils.NewGuidByteArray(), null! });
        response = await client.GetBatchAsync(client, "cache", badList);
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheGetBatchResponse.Error)response).ErrorCode);
    }
    ...
    public async Task GetBatchAsync_KeysAreByteArray_HappyPath()
    {
        ...
        CacheGetBatchResponse result = await client.GetBatchAsync(client, cacheName, keys);
    ...
    {
        CacheGetBatchResponse response = await client.GetBatchAsync(client, null!, new List<string>());
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheGetBatchResponse.Error)response).ErrorCode);
        response = await client.GetBatchAsync(client, "cache", (List<string>)null!);
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheGetBatchResponse.Error)response).ErrorCode);

        List<string> strings = new(new string[] { "key1", "key2", null! });
        response = await client.GetBatchAsync(client, "cache", strings);
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheGetBatchResponse.Error)response).ErrorCode);
    }
    ...
    public async Task GetBatchAsync_Failure()
    {
        ...
        CacheGetBatchResponse response = await simpleCacheClient.GetBatchAsync(client, cacheName, keys);
    ...
    {
        CacheSetBatchResponse response = await client.SetBatchAsync(client, null!, new Dictionary<byte[], byte[]>());
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheSetBatchResponse.Error)response).ErrorCode);
        response = await client.SetBatchAsync(client, "cache", (Dictionary<byte[], byte[]>)null!);
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheSetBatchResponse.Error)response).ErrorCode);

        var badDictionary = new Dictionary<byte[], byte[]>() { { Utils.Utf8ToByteArray("asdf"), null! } };
        response = await client.SetBatchAsync(client, "cache", badDictionary);
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheSetBatchResponse.Error)response).ErrorCode);
    }
    ...
    public async Task SetBatchAsync_ItemsAreByteArray_HappyPath()
    {
        ...
        await client.SetBatchAsync(client, cacheName, dictionary);
    ...
    {
        CacheSetBatchResponse response = await client.SetBatchAsync(client, null!, new Dictionary<string, string>());
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheSetBatchResponse.Error)response).ErrorCode);
        response = await client.SetBatchAsync(client, "cache", (Dictionary<string, string>)null!);
        Assert.Equal(MomentoErrorCode.INVALID_ARGUMENT_ERROR, ((CacheSetBatchResponse.Error)response).ErrorCode);
